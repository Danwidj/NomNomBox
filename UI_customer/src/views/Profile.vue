  ...
  <div class="nom-container py-8 nom-fade-in">
    ...
    <section class="max-w-4xl mx-auto mb-12">
      ...
      <div class="nom-card nom-card-hover">
        ...
        <div class="flex flex-col sm:flex-row gap-6 items-center sm:items-start mb-8">
          <div class="flex-1 text-center sm:text-left">
            ...
            <div class="mt-4">
              <router-link
                to="/edit-profile"
                class="nom-btn-outline inline-flex items-center justify-center"
              >
                <svg
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <!-- Contact Information -->
          <div class="space-y-4">
            <h4 class="text-lg font-medium border-b border-border pb-2 mb-4">
              Contact Information
            </h4>
          ...
          <div class="space-y-4">
            ...
            <div class="flex flex-wrap gap-2">
              ...
              <span
                v-if="!customer.dietary_preferences.length"
    ...
    <section class="max-w-4xl mx-auto">
      ...
      >
        <div class="flex flex-col items-center justify-center space-y-4">
          <svg
            class="w-12 h-12 text-muted-foreground/40"
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
          ...
          <p class="text-lg text-muted-foreground">
            {{
              activeTab === 'pending'
                ? 'No orders awaiting delivery scheduling.'
                : activeTab === 'active'
          ...
          <div v-else class="mt-2">
            <button @click="activeTab = 'active'" class="nom-btn-outline">
              View Active Orders
            </button>
          </div>
        </div>
          ...
          <div class="mb-6">
            <div class="flex justify-between items-center bg-muted/20 p-3 rounded-md">
              <div class="flex gap-4 items-center">
                <div
                  class="w-8 h-8 bg-secondary/10 rounded-full flex items-center justify-center text-secondary"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
      ...
      <div v-else-if="activeTab === 'active'" class="space-y-6 nom-fade-in">
        <div class="nom-card nom-card-hover" v-for="order in activeOrders" :key="order.orderId">
          <!-- Order Header -->
          <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
            <div>
              <h3 class="text-lg font-semibold">Order #{{ order.orderId }}</h3>
              ...
              <p v-if="order.scheduledDelivery" class="text-muted-foreground text-sm mt-1">
                ...
                {{ formatDeliveryDateTime(order.scheduledDelivery) }}
              </p>
            </div>
              ...
              <span
                :class="[
                  'px-3 py-1 rounded-full text-sm font-medium',
                  {
                    ...
                    'bg-amber-100 text-amber-800': order.status === 'Delivered',
      ...
      <div v-else-if="activeTab === 'delivered'" class="space-y-6 nom-fade-in">
        <div class="nom-card nom-card-hover" v-for="order in deliveredOrders" :key="order.orderId">
          <!-- Order Header -->
          <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
            <div>
              <h3 class="text-lg font-semibold">Order #{{ order.orderId }}</h3>
              ...
              <p class="text-muted-foreground text-sm">
                Delivered:
                {{ order.scheduledDelivery ? formatDate(order.scheduledDelivery) : 'Unknown Date' }}
                <!-- this line worked -->
              </p>
            </div>
...
export default {
  ...
  setup() {
    ...
    const pendingOrders = computed(() => {
      return sortedOrders.value.filter(
        (order) =>
    ...
    const activeOrders = computed(() => {
      return sortedOrders.value.filter((order) => {
        ...
        return (
          (orderStatus === 'paid' &&
            (order.scheduledDelivery ||
              localStorage.getItem(`order_${order.orderId}_deliveryTime`))) ||
    ...
    const checkDeliveryStatus = async (deliveryId, orderId) => {
      try {
        const response = await fetch(`http://localhost:5010/delivery/${deliveryId}`)
        ...
        if (data.code === 200 && data.data) {
          ...
          if (
            deliveryData.status === 'Delivered by Driver' ||
            deliveryData.status === 'Received by Customer'
          ) {
    ...
    const confirmDelivery = async (orderId) => {
      try {
        ...
        const deliveryResponse = await fetch(`http://localhost:5005/deliveries/${deliveryId}`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
          },
